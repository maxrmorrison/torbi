...
__global__ void viterbi_forward_kernel(
    ...
    __shared__ float posterior_cache_0[1440];
    __shared__ float posterior_cache_1[1440];

    float *posterior_current = posterior_cache_0;
    float *posterior_next = posterior_cache_1;
    ...
    for (int t=1; t<frames; t++) {
        ...
        for (int j=warp_id; j<states; j+=NUM_WARPS) {
            ...
            for (int i=thread_warp_id+WARP_SIZE; i<states; i+=WARP_SIZE) {
                ...
                // float new_value = posterior[(t-1)*states + i] + transition[j*states + i];
